...
var pK = {
  ...
  parse: function(N1S) {
    ...
    var s1S = ' ';
    function i1S(Sp) {
      throw { name: 'JSONError', message: Sp, Qn: b1S - 1, text: N1S };
    }
    function A1S() {
      s1S = N1S.charAt(b1S);
      b1S += 1;
...
function l1(vKS, wKS) {
  for (var NKS in vKS) if (vKS[NKS] == wKS) return true;
  return false;
}
function I(iKS) {
...
function _(dhS, khS, hhS) {
  return function(mM) {
    ...
    if (CM == dhS && CM != yM && !f1(CM, yM)) {
      for (var jM in khS) if (yM == khS[jM]) return;
      hhS(new z1(dhS, mM));
    }
  };
}
function nj(MhS, WhS, GhS) {
  return function() {
    for (var aM in WhS) if (window.event.toElement == WhS[aM]) return;
...
function f1(_hS, BmS) {
  try {
    if (!BmS) return false;
    while (BmS.parentNode) if ((BmS = BmS.parentNode) == _hS) return true;
    return false;
  } catch (JhS) {
    return false;
  }
...
function e1(zmS) {
  YK('getState', {}, function(uM) {
    if (zmS) zmS(uM);
  });
}
function H1(SmS, umS) {
  YK('storeState', { rW: SmS }, function(SM) {
    if (umS) umS(SM);
  });
}
function yX(PmS, cmS) {
  YK('getSetting', { vW: PmS }, function(PM) {
    if (cmS) cmS(PM.value);
  });
}
function DX(gmS) {
  YK('getSettings', {}, function(cM) {
    if (gmS) gmS(cM);
  });
}
function NK(vmS, NmS, wmS) {
  YK('storeSetting', { key: vmS, value: NmS }, function(gM) {
    if (wmS) wmS(gM);
  });
}
function C(EmS, imS) {
  YK('storeSettings', { AW: EmS }, function(wM) {
    if (imS) imS(wM);
  });
}
function bK(AmS, bmS, XmS) {
  YK('restoreTabsMeta', { Gj: AmS, xn: bmS }, function(NM) {
    if (XmS) XmS(NM);
  });
}
function UK(qmS) {
  YK('upgradeAvailable', {}, function(vM) {
    if (qmS) qmS(vM.value);
  });
}
function WK(OmS) {
  YK('getAllMessages', {}, function(iM) {
    if (OmS) OmS(iM);
  });
}
MK(function() {
...
function SX(dmS, smS) {
  var hmS = {};
  YK('getExtensionKey', {}, function(AM) {
    hmS['key'] = AM;
    var EM = [];
    hmS['tabGroups'] = dmS;
    aX(L1 + '/api/createPage', hmS, function(HJ) {
      YK('createNewTab', { url: HJ['url'] }, function() {
        smS();
      });
    });
  });
...
function QX(j2S) {
  ...
  u2S.style.display =
    j2S['label'] && R1(j2S['label']) != '' ? 'inline-block' : 'none';
  c2S.appendChild(
    (function() {
    ...
    (function() {
        ...
        (function() {
          ...
          pJ.appendChild(
            document.createTextNode(
              x1('createdPreceedingDate') +
                ' ' +
                new Date(j2S['createDate']).toLocaleString(),
            ),
          );
          return pJ;
        })(),
      ...
      GM.appendChild(
        (function() {
          ...
          PJ.appendChild(uJ);
          if (k6S)
            uJ.style['box-shadow'] = 'rgb(221, 221, 221) 1px 1px 1px 1px';
          ...
          uJ.appendChild(
            SJ(x1('nameThisTabGroup'), 6, function(Ye) {
              if (!j2S['label']) {
                o2S.rn('', false, true);
              }
              u2S.style.display = 'inline-block';
          ...
          uJ.appendChild(
            SJ(
              j2S['locked'] ? x1('unlockTabGroup') : x1('lockTabGroup'),
              6,
              function(Qe, re) {
          ...
          uJ.appendChild(
            SJ(
              j2S['starred'] ? x1('unstarTabGroup') : x1('starTabGroup'),
              6,
              function(He, Ze) {
          ...
          uJ.appendChild(
            SJ(x1('help'), 0, function(Te) {
              YK('createNewTab', { url: L1 + '/help' });
            }),
          );
          PJ.onclick = function() {
  ...
  for (var y2S in j2S['tabsMeta']) {
    ...
    var K2S = oK(T2S['url']);
    if (K2S == 'news.ycombinator.com')
      z2S = 'https://news.ycombinator.com/favicon.ico';
    ...
    a2S.appendChild(
      document.createTextNode(W1(T2S['title'] ? T2S['title'] : 'Untitled')),
    );
    a2S.innerHTML += '&nbsp';
    var C2S = T2S['url'];
    if (C2S.indexOf('://') == -1 && C2S.indexOf(':') == -1)
      C2S = 'http://' + C2S;
    C2S = kK(C2S);
    a2S.href = C2S;
      ...
      return function(GJ) {
        ...
        yX('restoreRemoval', function(Re) {
          if (_J || j2S['locked'] || Re == 'keep') {
            if (WJ) {
              YK('createNewTabsInNewFocusedWindow', { Gj: [QF] });
            } else {
    ...
    var Y2S = (function(RF, fF) {
      rX.oW(
        RF,
        { BW: j2S['id'], pW: fF['id'], Jj: r2S, lW: fF },
        RF,
        'tabLink',
        function() {},
        function() {},
...
function XK() {
  ...
  E2S.appendChild(
    (function() {
      ...
      zF.appendChild(
        Tj(uF, 14, function() {
          YK('sendAllTabsInAllWindowsToOneTabAction', {}, function() {
            e1(function(ry) {
              u1(ry);
            });
          });
  ...
  E2S.appendChild(
    (function() {
      ...
      SF.appendChild(
        Tj(x1('shareAllAsWebPage'), A2S, function(rq) {
          I(rq);
          ...
          e1(function(xe) {
            ...
            if (Qq.length == 0) {
              ...
              SX(Ke, function() {
                I(rq);
                rq.appendChild(
                  document.createTextNode(x1('shareAllAsWebPage')),
                );
              });
            }
          });
  ...
  E2S.appendChild(
    (function() {
      ...
      PF.appendChild(
        Tj(x1('exportImportUrls'), A2S, function() {
          YK('showOrRefreshAndFocusScriptPage', { url: 'import-export.html' });
        }),
      );
      return PF;
    })(),
  ...
  E2S.appendChild(
    (function() {
      ...
      cF.appendChild(
        Tj(x1('options'), A2S, function() {
          YK('showOrRefreshAndFocusScriptPage', { url: 'options.html' });
        }),
      );
      return cF;
    })(),
  ...
  E2S.appendChild(
    (function() {
      ...
      wF.appendChild(
        Tj(x1('about') + ' / ' + x1('feedback'), A2S, function() {
          YK('createNewTab', { url: L1 });
        }),
      );
      return wF;
    })(),
...
function yj() {
  if (false && k6S) {
    ...
    window['WebFontConfig'] = {
      google: { families: ['Open Sans:400,600,300,700'] },
      active: function() {
        window.R = true;
      },
      inactive: function() {
  ...
  var X2S = function() {
    setTimeout(function() {
      YK('getExtensionKey', {}, function(me) {
        aX(
          L1 + '/api/newVersionCheck',
          { version: y1, extensionKey: me },
          function(Hy) {
...
function eX() {
  ...
  UK(function(AF) {
    if (AF) {
      ...
      XF.appendChild(cK());
      XF.appendChild(
        document.createTextNode(' is available - please click the '),
      );
      XF.appendChild(RX(28, 3));
      XF.appendChild(
        document.createTextNode(
          ' menu button in the top right of your browser and then click "Update Google Chrome". If that option is not available, click "About Google Chrome" followed by "Relaunch".',
        ),
      );
    }
  });
